...
plot(roc_obj, main="Curva ROC para el Modelo de Regression Logística")
```
...
```{r}
# Ajustar el modelo SVM con un kernel lineal y especificando un valor de costo C
# svm_model <- svm(TARGET ~ CODE_GENDER + FLAG_OWN_CAR + FLAG_OWN_REALTY + CNT_CHILDREN +
#                 AMT_INCOME_TOTAL + AMT_CREDIT + AMT_ANNUITY + AMT_GOODS_PRICE +
#                 DAYS_BIRTH + DAYS_EMPLOYED + EXT_SOURCE_2 + EXT_SOURCE_3,
#                 data = training_set, 
#                 method = "C-classification", 
#                 kernel = "linear",  # Cambiado a kernel lineal
#                 cost = 1)  # Valor de costo C, ajusta según sea necesario
...
```{r}
# Matriz de Confusión
# confusionMatrix <- confusionMatrix(as.factor(svm_predictions), as.factor(test_labels))
# print(confusionMatrix)
```
...
nb_model <- naiveBayes(TARGET ~ CODE_GENDER + FLAG_OWN_CAR + FLAG_OWN_REALTY + CNT_CHILDREN + 
                       AMT_INCOME_TOTAL + AMT_CREDIT + AMT_ANNUITY + AMT_GOODS_PRICE +
                       DAYS_BIRTH + DAYS_EMPLOYED + EXT_SOURCE_2 + EXT_SOURCE_3, 
                       data = training_set)
...
nb_predictions_raw <- predict(nb_model, newdata = test_set, type = "raw")
```
...
```{r}
# Matriz de Confusión
confusionMatrix <- confusionMatrix(as.factor(nb_predictions), as.factor(test_set$TARGET))
confusionMatrix_data <- confusionMatrix(as.factor(nb_predictions), as.factor(test_set$TARGET))
print(confusionMatrix)
```
...
```{r message=FALSE, warning=FALSE}
# Calcular AUC
roc_obj <- roc(response = as.numeric(test_set$TARGET), predictor = nb_predictions_raw[,2])
auc_value <- auc(roc_obj)
cat("El valor AUC es:", auc_value, "\n")
```

-   **ROC Curve**
```{r}
# Curva ROC
plot(roc_obj, main = "Curva ROC para el Modelo Naive Bayes", lwd = 2)
...
rf_model <- randomForest(TARGET ~ CODE_GENDER + FLAG_OWN_CAR + FLAG_OWN_REALTY + CNT_CHILDREN + 
                         AMT_INCOME_TOTAL + AMT_CREDIT + AMT_ANNUITY + AMT_GOODS_PRICE +
                         DAYS_BIRTH + DAYS_EMPLOYED + EXT_SOURCE_2 + EXT_SOURCE_3, 
                         data = training_set, ntree = 100, mtry = 3, importance = TRUE)

# Hacer predicciones en el conjunto de prueba
rf_predictions <- predict(rf_model, newdata = test_set)

# Generar predicciones probabilísticas para la clase de interés
rf_predictions_prob <- predict(rf_model, newdata = test_set, type = "prob")[,2]
```
...
-   **Matriz de Confusión**
```{r}
confusionMatrix <- confusionMatrix(as.factor(rf_predictions), as.factor(test_set$TARGET))
print(confusionMatrix)
```
...
```{r message=FALSE, warning=FALSE}
# Calcular AUC
roc_obj_rf <- roc(as.numeric(test_set$TARGET), rf_predictions_prob)
auc_value_rf <- auc(roc_obj_rf)
cat("El valor AUC para Random Forest es:", auc_value_rf, "\n")
```

-   **ROC Curve**
```{r}
# Curva ROC
plot(roc_obj_rf, main = "Curva ROC para el Modelo Random Forest")
abline(a = 0, b = 1, lty = 2, col = "gray")
```

## Evaluación y Selección de Modelo de Clasificación

### Insights Clave de los Modelos

#### Regresión Logística
- **Desempeño General**: Con un AUC de 0.7275, la regresión logística tiene un rendimiento moderado en la distinción entre las clases.
- **Coeficientes Significativos**: `CODE_GENDER`, `FLAG_OWN_CAR`, y las fuentes externas (`EXT_SOURCE_2`, `EXT_SOURCE_3`) son predictores significativos de la dificultad de pago.
- **Baja Sensibilidad**: El modelo tiene una especificidad cercana a cero, lo que indica una baja sensibilidad para detectar la clase positiva.

#### Árboles de Decisión (CART)
- **AUC Inferior**: Con un AUC de 0.5925, sugiere una capacidad limitada para diferenciar entre las clases en comparación con otros modelos.
- **Balance**: Pese a su menor AUC, los árboles de decisión pueden ser más fáciles de interpretar y explicar.

#### K-Vecinos Más Cercanos (KNN)
- **Cercanía al Azar**: Un AUC de 0.5168 indica que el modelo está actuando apenas mejor que un lanzamiento de moneda al azar en este conjunto de datos.

#### Naive Bayes
- **Rendimiento Mejorado**: Con un AUC de 0.6937, Naive Bayes muestra un mejor rendimiento que KNN pero aún es inferior a la regresión logística.
- **Predicciones Equilibradas**: A pesar de su sencillez, presenta un mejor equilibrio entre sensibilidad y especificidad comparado con la regresión logística.

#### Random Forest
- **Mejor Modelo**: Tiene el AUC más alto con 0.7028, lo que sugiere una mejor capacidad de clasificación que los modelos anteriores.
- **Estabilidad**: Dado que incorpora múltiples árboles de decisión, es menos propenso al sobreajuste y puede manejar mejor las interacciones complejas entre variables.

#### Puntos Importantes a Considerar
Al seleccionar un modelo, se deben equilibrar la precisión de la predicción (AUC), la interpretabilidad y la complejidad computacional. Si bien Random Forest lidera en rendimiento, la regresión logística puede ofrecer un mejor equilibrio entre interpretabilidad y precisión. Naive Bayes ofrece un compromiso decente cuando se necesita un modelo simple y efectivo. KNN y los árboles de decisión pueden no ser adecuados para este problema específico dado su rendimiento en este conjunto de datos.

## Conclusiones y Análisis

Los modelos de clasificación evaluados muestran un rango diverso de rendimientos y características. La elección final del modelo debe basarse no solo en la precisión, sino también en la capacidad de interpretación de los resultados.

La **regresión logística** ofrece una interpretación clara de las relaciones entre las variables y la probabilidad de incumplimiento en pagos, a pesar de su sensibilidad limitada. Su AUC de 0.7275 sugiere que hay espacio para mejorar en términos de distinguir entre las clases.

El modelo **CART** tiene el AUC más bajo, lo que plantea dudas sobre su capacidad para clasificar con precisión en este conjunto de datos. Sin embargo, ofrece una gran interpretabilidad, que podría ser valiosa para explicar las decisiones a las partes interesadas.

El desempeño de **KNN** está cerca del azar, lo que sugiere que no es adecuado para este problema particular, quizás debido a la alta dimensionalidad de los datos o la necesidad de una métrica de distancia más refinada.

**Naive Bayes**, a pesar de su suposición de independencia entre características, ofrece un rendimiento decente y puede ser una opción rápida y robusta cuando la complejidad del modelo es una preocupación.

**Random Forest** emerge como el modelo más prometedor con el AUC más alto, lo que indica su fuerza en la captura de interacciones complejas entre variables y su robustez general. A pesar de su complejidad y el costo computacional más alto, podría ser la mejor elección si la prioridad es la precisión.

En resumen, el modelo de **Random Forest** parece ser la opción más sólida para este análisis de clasificación de solicitudes bancarias, equilibrando precisión y manejo de la complejidad de los datos. Sin embargo, para aplicaciones donde la interpretabilidad es crucial, la **regresión logística** podría ser más adecuada. Los resultados subrayan la importancia de considerar no solo las métricas de rendimiento, sino también la aplicabilidad práctica y las limitaciones operativas al seleccionar un modelo para implementar.
...
-   **Modelo de Random Forest**: Elegido por su mayor AUC (0.7028), lo que indica una fuerte capacidad discriminativa.
-   **Balance de Precisión**: Ofrece un buen compromiso entre la sensibilidad y la resistencia al sobreajuste.
-   **Variables Críticas**: Las fuentes externas (`EXT_SOURCE_2`, `EXT_SOURCE_3`) y ciertos demográficos (`CODE_GENDER`, `FLAG_OWN_CAR`) son predictores clave.
-   **Interpretación de Random Forest**: Sugiere patrones socioeconómicos y de crédito significativos que afectan la predicción de dificultades de pago.
-   **Sensibilidad y Especificidad**: Alta sensibilidad para detectar clientes sin problemas de pago, pero con espacio para mejorar en la identificación de clientes en riesgo.
-   **Uso de Información Externa**: Resalta la importancia de las evaluaciones de crédito externas en la predicción del riesgo.

### c. Interpretación de Resultados Estimados
Brevemente, interpretar los resultados estimados.

-   **Ponderación de Fuentes Externas**: La elevada importancia de `EXT_SOURCE_2` y `EXT_SOURCE_3` en el modelo Random Forest indica que los puntajes de crédito externos y otros datos de terceros son fundamentales para evaluar la solvencia de los clientes.
-   **Influencia Demográfica**: Variables como `CODE_GENDER` y `FLAG_OWN_CAR` reflejan cómo factores sociodemográficos pueden influir en la capacidad de pago de los clientes, posiblemente debido a diferencias en la estabilidad de ingresos y compromisos financieros.
-   **Propiedad de Vehículos**: El peso asignado a `FLAG_OWN_CAR` sugiere que poseer un automóvil puede estar relacionado con un mayor riesgo crediticio, posiblemente por los gastos asociados al mantenimiento y la financiación de vehículos.
-   **Significancia Estadística**: Los coeficientes de la regresión logística muestran que ciertas características económicas del cliente (como `AMT_INCOME_TOTAL`, `AMT_CREDIT`) tienen una relación directa y estadísticamente significativa con la probabilidad de incumplimiento.
-   **Riesgo de Crédito**: Estos resultados permiten al banco identificar qué factores considerar más críticamente al evaluar las solicitudes de crédito, mejorando así las estrategias de mitigación de riesgos y personalización de productos financieros.
