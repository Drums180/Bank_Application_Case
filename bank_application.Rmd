...
## Instrucciones
A partir de la asignación durante la sesión de clase de las siguientes bases de datos i) bank_application_data o ii) bank_marketing_data, realizar las instrucciones 1 – 6. En el
desarrollo del archivo de R-Markdown, por favor incluir data storytelling de los resultados del
análisis exploratorio de los datos (EDA) así como la interpretación de los resultados estimados.

## Lectura Sugeridas:
- What is Classification in Machine Learning and Why Is It Important?
https://emeritus.org/blog/artificial-intelligence-and-machine-learning-classification-in-machine-
learning/
...
El **aprendizaje supervisado** o **Supervised Machine Learning** es una categoría del machine learning donde el modelo se entrena con un conjunto de datos etiquetados, es decir, para cada entrada en el conjunto de entrenamiento, ya conocemos la salida correcta. El objetivo es que el modelo aprenda a predecir la salida para nuevas entradas basándose en esta experiencia de entrenamiento. En análisis de clasificación, se utiliza para predecir la categoría o clase a la que pertenece una nueva entrada. Algunas aplicaciones incluyen:
...
- iii) Respecto a la selección de los resultados de los modelos de clasificación ¿Qué es la matriz de
confusión? ¿Qué es el estadístico Kappa? ¿Cuál es la relación entre AUC y ROC Curve?
...
- **Relación entre AUC y ROC Curve:** La curva ROC evalúa cómo de bien un modelo distingue entre categorías a distintos niveles de decisión. AUC, el área bajo esta curva, resume en un número cuán efectivo es el modelo.
...
```
**Insights de Analisis Descriptivo**
- **TARGET**: La mayoría de los préstamos en la muestra no presentan dificultades de pago (el 92% tienen un TARGET de 0).
- **CNT_CHILDREN**: La distribución muestra que la mayoría de los clientes no tienen hijos, con una mediana de 0. Sin embargo, hay casos con hasta 19 hijos, lo cual es inusual y podría ser un outlier.
- **AMT_INCOME_TOTAL**: Existe una gran variabilidad en los ingresos de los clientes, desde un mínimo de 25,650 hasta un máximo de 117,000,000. Esto sugiere una alta disparidad económica entre los solicitantes de préstamos.
...
- **Edad de los Solicitantes**: El histograma muestra una distribución relativamente uniforme de edades entre los solicitantes, concentrándose principalmente entre los 30 y 60 años. Esto sugiere que la mayoría de los solicitantes están en su etapa laboralmente activa.
...
- **Correlación entre Variables**: La matriz de correlación sugiere que ciertas variables como la edad y los días de empleo están altamente correlacionadas. Esto podría ser útil para simplificar el modelo predictivo al eliminar variables redundantes.
...
- **Reemplazo de Outliers**: Imputar outliers extremos con medianas para evitar distorsiones en el modelo. (Realizado antes de Analisis Exploratorio para mejor visualización y analisis)
...
- **Edad (DAYS_BIRTH)**: Al observar la edad de los solicitantes, podríamos esperar que los más jóvenes, aún sin establecer un historial crediticio sólido, puedan tener más probabilidades de incumplimiento. Por otro lado, los solicitantes mayores podrían ser más estables financieramente y tener menos probabilidades de incumplimiento.
...
El impacto de cada variable será evaluado a través de la significancia de sus coeficientes en el modelo y su contribución a la precisión de las predicciones. Además, si bien la correlación entre las variables y `TARGET` no mostro impactos directos durante el analisis de la matriz de correlación, dichos impactos pueden pasar inadvertidos debido a su naturaleza no lineal o interacciones con otras variables. Por lo tanto, se emplearán técnicas de modelización que puedan capturar efectos no lineales y de interacción, como árboles de decisión, modelos de ensamble o redes neuronales, que pueden revelar relaciones complejas y ofrecer una mejor interpretación del riesgo crediticio.
...
```{r}
# Estadístico Kappa
kappa_value <- kappa_stats[[1]]
cat("Kappa Statistic:", kappa_value, "\n")
```

- **AUC**
```{r message=FALSE, warning=FALSE}
# AUC
roc_response <- roc(response = test_set$TARGET, predictor = as.numeric(tree_predicted_probs))
...
```{r}
# ROC Curve
plot(roc_response, main = "ROC Curve for Decision Tree Model")
```
...
# Ajustamos K-Means sin la variable objetivo
kmeans_result <- kmeans(scale(training_set[, -which(names(training_set) == "TARGET")]), centers = 2, nstart = 25)

# Añadir la asignación de cluster al conjunto de datos de entrenamiento
training_set$cluster <- kmeans_result$cluster

# Evaluar la calidad de los clusters
# Silhouette score
silhouette_score <- silhouette(kmeans_result$cluster, dist(scale(training_set[, -which(names(training_set) == "TARGET")])))
...
fviz_cluster(kmeans_result, data = scale(training_set[, -which(names(training_set) == "TARGET")]))

# Cruzar los clusters con la variable TARGET
table(training_set$TARGET, training_set$cluster)

# Puedes intentar analizar las estadísticas descriptivas de cada cluster
aggregate(. ~ cluster, data = training_set, FUN = mean)
```

**Creación de Modelo**
```{r}
# Asumiendo que sabemos que hay dos clusters (por ejemplo, TARGET es binario)
set.seed(123) # Para reproducibilidad
kmeans_model <- kmeans(training_set[, -which(names(training_set) == "TARGET")], centers = 2)

# Asignar los clusters a las etiquetas de clase más cercanas
# Esto supone que ya se ha realizado algún tipo de análisis para entender cómo se corresponden los clusters con las etiquetas de clase TARGET
cluster_labels <- as.factor(ifelse(kmeans_model$cluster == 1, 0, 1)) # Sustituir con la lógica de asignación adecuada

# Añadir las etiquetas de cluster al conjunto de datos de entrenamiento para la evaluación
training_set$cluster_labels <- cluster_labels
```
...
```{r}
# Matriz de Confusión
confusionMatrix <- table(Predicted = training_set$cluster_labels, Actual = training_set$TARGET)
print(confusionMatrix)
```
...
- **AUC**
```{r}
# Calculamos la distancia al centroide para usar como proxy de probabilidad
distances_to_centroid <- sqrt(rowSums((training_set[, -which(names(training_set) == "TARGET")] - kmeans_model$centers[kmeans_model$cluster,])^2))
normalized_distances <- (distances_to_centroid - min(distances_to_centroid)) / (max(distances_to_centroid) - min(distances_to_centroid))
proxy_probabilities <- 1 - normalized_distances

# AUC utilizando la distancia al centroide como probabilidad
roc_response <- roc(response = training_set$TARGET, predictor = proxy_probabilities)
auc_value <- auc(roc_response)
cat("AUC:", auc_value, "\n")
```
...
```{r}
# ROC Curve
plot(roc_response, main = "ROC Curve using K-Means Clustering Proxy Probabilities")
```
